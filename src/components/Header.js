...
function Header({ onThemeChange, isDarkMode }) {
  ...
  // États pour gérer l'affichage du header
  const [showHeader, setShowHeader] = useState(true);
  const [activeSection, setActiveSection] = useState('profile'); // Track active section
  
  // Références pour stocker la dernière position de défilement et l'état de la souris
  const lastScrollY = useRef(0);
  const isMouseNearTop = useRef(false);

  // Seuil pour la détection de la souris proche du haut (en pixels)
  ...
  const handleScroll = throttle(() => {
    ...
    if (activeSection === 'profile') {
      // Toujours montrer le header sur la section 'profile'
    ...
    if (isMouseNearTop.current) {
      // Si la souris est près du haut, garder le header visible
  ...
  }, 200); // Throttling à 200ms

  // Fonction pour gérer le mouvement de la souris avec throttling
  const handleMouseMove = throttle((e) => {
    if (e.clientY < MOUSE_THRESHOLD) {
      ...
      if (isMouseNearTop.current) {
        ...
        // Après avoir quitté la zone, déterminer si le header doit être affiché ou masqué
        handleScroll();
      }
    }
  }, 200); // Throttling à 200ms
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      handleMouseMove.cancel(); // Annule tout appel throttled en attente
    };
  }, [activeSection]); // Dépendance pour re-run l'effet quand activeSection change
